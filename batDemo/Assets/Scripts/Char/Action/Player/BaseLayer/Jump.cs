...
public class Jump : ActionBase
{
    ...
    private void onBeginJump(int frame=0){
        if(player.charData.aimState==GameEnum.AimState.Null){
            ...
            player.cameraCtrl.smooth=5;
        }
        if(player.charData.isDashing){
            if(player.charData.aimState==GameEnum.AimState.Null){
               player.cameraCtrl.SetFOV(80);
            }
       //     CameraManager.Instance.postLayer.enabled=true;
            this.obj.moveSpeed=player.charData.DashSpeed;
            this.movePart.speed=player.charData.DashSpeed; 
        }else{
            if(player.charData.aimState==GameEnum.AimState.Null){
               player.cameraCtrl.ResetFOV();
            }
    ///       CameraManager.Instance.postLayer.enabled=false;
            this.obj.moveSpeed=player.charData.RunSpeed;
            this.movePart.speed=player.charData.RunSpeed;
        }
        if(player.charData.aimState!=GameEnum.AimState.Null){
            //DebugLog.Log("player");
            player.AniPlay(GameEnum.AniLabel.Down_Jmp_Base_A_Fall,0,0.333f,1f*this.speed,0.25f,3);
        }else{
    ...
    private void onJumpToGround(object[] param=null) {
        ...
        //Character character=this.obj as Character;
         player.SetCharacterCtrlHeight(1.8f);
         if(player.charData.aimState!=GameEnum.AimState.Null){
              player.AniPlay(GameEnum.AniLabel.DownIdle,0,0.1f,1f,0.8f,3);
         }
        Vector3 dirp=player.dirPos;
