...
public class JoyController : Controller
{
    ...
    public void  initCharDir(){
     ...
     //   DebugLog.Log(this.onJoyTouch);
        if(this.onJoyTouch){
            if(_IsDraging&&JoyAngle>=-70&&JoyAngle<=70&&_last_H!=_player.cameraCtrl.GetH()){
        ...
        }else if(isDashing&&_last_H!=_player.cameraCtrl.GetH()){
            _last_H=_player.cameraCtrl.GetH();
             Vector3 worldDir =this._player.GetMovePart().getRotation(_last_H);
             this.SendMessage(CharEvent.OnJoy_Move,new object[]{worldDir,isDashing,false,this.JoyAngle});
        }
    ...
    private void OnSprint(object[] data){
        ...
        _player.charData.isDashing=isDashing;
       if(isDashing&&_player.GetCurStateID()==GameEnum.CharState.Char_Idle && _player.charData.currentBaseAction!=GameEnum.ActionLabel.Run&&_player.charData.currentBaseAction!=GameEnum.ActionLabel.Dash){
           this.SendMessage(CharEvent.OnJoy_Move,new object[]{this._player.gameObject.transform.forward,isDashing,false,this.JoyAngle});
       }else  if(!isDashing&&_player.GetCurStateID()==GameEnum.CharState.Char_Idle && (_player.charData.currentBaseAction==GameEnum.ActionLabel.Run ||_player.charData.currentBaseAction==GameEnum.ActionLabel.Dash)){
           this.SendMessage(CharEvent.OnJoy_Up);
       }
    }
    private void onTouchState(object[] data){
        ...
        if(this.onJoyTouch&&!_IsDraging && (_player.charData.currentBaseAction==GameEnum.ActionLabel.Run || _player.charData.currentBaseAction==GameEnum.ActionLabel.Dash) ){
 //              DebugLog.Log("changeDir");
    ...
    private void  onTouchMove(object[] data){
      ...
      //  delta.Normalize();
	    _player.cameraCtrl.onTouchMove(delta);
