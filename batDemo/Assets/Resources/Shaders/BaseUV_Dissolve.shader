...
Shader "Blue/Character/BaseUV_Dissolve" 
{
	Properties 
	{
	...
	SubShader 
	{
		...
		Pass 
		{
			...
			fixed4 frag_surf (VertexOutput IN) : COLOR 
			{
            	...
            	scroll_col.rgb *= _ScrollPower;
            	scroll_col *= _ScrollColor;
            	finalColor.rgb += scroll_col.a*scroll_col.rgb;
			...
			//  	finalColor.rgb  *= step(0.001, _DissolveAmount)*step(clipTemp.y, _DissolveStart)*(10 * clipTemp.y/_DissolveStart-1) +1;
		    	...
		    	finalColor.rgb  *=_DissolveAmount*step(clipTemp.y, _DissolveStart)*(10 * clipTemp.y/_DissolveStart-1) +1;
