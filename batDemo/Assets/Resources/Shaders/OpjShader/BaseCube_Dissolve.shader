...
Shader "Blue/Character/BaseCube_Dissolve" 
{
	Properties 
	{
	    ...
	    _ShadowDir("Shadow Direction", Vector) = (0, 0, 0, 0)
		...
		_DissolveAmount ("Dissolve Amount", Range (0.001, 1)) = 0.001
		_DissolveStart("Dissolve Start", float) = 0.3
	}
	...
	SubShader 
	{
		...
		Pass 
		{
			...
			fixed4 frag_surf (VertexOutput IN) : COLOR 
			{
				IN.normal = normalize(IN.normal);
			  	...
			  	finalColor.rgb += texColor*control.b *texCUBE (_CubeTex, IN.worldRefl)*_CubeValue;
		...
		//	  	finalColor.rgb  *= step(0.001, _DissolveAmount)*step(clipTemp.y, _DissolveStart)*(3*clipTemp.y/_DissolveStart-1) +1;
			 
			    finalColor.rgb  *=_DissolveAmount* step(clipTemp.y, _DissolveStart)*(3*clipTemp.y/_DissolveStart-1) +1;
