...
namespace EmmyLua
{
	...
	class EmmyLuaService
	{
		...
		{
            return CustomSettings.customTypeList.Select(x=>x.type).Union(CustomSettings.ignoreTypes).ToArray();
   //         var unityTypes = from assembly in AppDomain.CurrentDomain.GetAssemblies()
			//	where !(assembly.ManifestModule is ModuleBuilder)
			//	      && assembly.FullName != "UnityEditor"
			//	from type in assembly.GetTypes()
			//	where type.BaseType != typeof(MulticastDelegate)&&!type.Name.EndsWith("Wrap")
			//	      && !type.IsInterface 
			//	      && !type.IsEnum
			//	      //&& !type.IsGenericType
