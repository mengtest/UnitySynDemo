	...
	void _generateLuaScript()
	{
		...
		if (!File.Exists(path1))
		{
			using (StreamWriter sw = File.CreateText(path1))
			{
				...
				sb.AppendLine("--自动生成，逻辑代码不要写在这里");
				//添加function:init
				sb.AppendLine("function " + name + ":Init()");
				sb.AppendLine("end\n");
				//添加function：baseCreateView()
...
class LuaObject
{
    ...
    public void Update(ref string text)
    {
        if (oldname != "" && name != "")
        {
            ...
            if (r.IsMatch(text))
            {
                DebugLog.LogError(string.Format("该lua文件已经有了名为{0}的变量", name));
                name = oldname;
                oldname = "";
                return;
            }
...
class DrawLuaInspector
{
    ...
    public static void OnInspectorGUI(Transform transform)
    {
        ...
        else
        {
            ...
            {
                if (GUILayout.Button("生成预制和lua代码"))
                {
                    if (EditorUtility.DisplayDialog("创建", "是否创建视图" + transform.name, "确定", "取消"))
                    {
