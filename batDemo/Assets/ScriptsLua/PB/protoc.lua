...
function Parser:parsefile2(name)
   local info = self.loaded[name]
   if info then return info end
   local errors = {}
   -- for _, path in ipairs(self.paths) do
      -- local fn = path ~= "" and path.."/"..name or name
      local tempPath = string.gsub(name,".pb",".txt")
      local fh = ResHelper.LoadProtoPbBytes(tempPath)
      if fh then
         info = self:parse(io:read(fh), name)
         return info
      end
      insert_tab(errors, err or name..": ".."unknown error")
   -- end
   if self.import_fallback then
      info = self.import_fallback(name)
   end
...
function Parser:compilefile2(fn)
   local set = do_compile(self, self.parsefile2, self, fn)
   return pb.encode('.google.protobuf.FileDescriptorSet', set)
end
...
function Parser:loadfile2(fn)
   local ret, pos = pb.load(self:compilefile2(fn))
   if ret then return ret, pos end
   error("load failed at offset "..pos)
end
