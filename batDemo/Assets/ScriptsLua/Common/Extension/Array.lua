    ...
    local mt = {
        ...
        __newindex = function(t, k, v)
            if nil == __array__[k] then
                log(string.format('warning : [%s] index out of range.', tostring(k)))
                return
            end
            if nil == v then
                log(string.format('warning : can not remove element by using  `nil`.'))
                return
            end
            __array__[k] = v
        end
