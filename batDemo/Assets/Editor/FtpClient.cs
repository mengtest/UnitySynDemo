...
namespace MiniEditor
{
    public class FtpClient
    {
        ...
        /// <returns></returns>
        public bool foldersUpload(string sourceFolder, string destFolder)
        {
            DebugLog.Log("foldersUpload", sourceFolder, destFolder);
            bool isFolderFlag = false;
            if (isFlag)
            {
                baseFolderPath = sourceFolder.Substring(0, sourceFolder.LastIndexOf("/"));
                //baseFolderPath = sourceFolder;
                isFlag = false;
            }
            //DebugLog.LogError("baseFolderPath", baseFolderPath);
            ...
            if (selectFolderName != null)
            {
                ...
                //}
                //DebugLog.Log("检查文件夹是否存在", ftpDirectory);
            ...
            try
            {
                ...
                if (files.Length > 0)
                {
                    foreach (string s in files)
                    {
                        ...
                        //DebugLog.LogError("selectFolderName", selectFolderName);
        ...
        {
            DebugLog.Log("fileUpload", localFile, ftpPath, ftpFileName);
            bool success = false;
            ...
            try
            {
                ...
                //fileDelete(ftpPath, ftpFileName);
            ...
            {
                DebugLog.LogError("上传失败", ftpRootURL + ftpPath + ftpFileName, e.Message);
                success = false;
            }
            finally
            {
        ...
        public bool fileDelete(string ftpPath, string ftpName)
        {
            ...
            {
                DebugLog.LogError("文件删除失败", ftpRootURL + ftpPath + ftpName, e.Message);
                success = false;
            }
            finally
            {
        ...
        public void CreateFtpDirectory(string ftpDirectory)
        {
            try
            {
                DebugLog.Log("CreateFtpDirectory", ftpDirectory);
            ...
            {
                DebugLog.LogWarning("FTP创建目录失败" + ex.Message);
            }
        }
