...
public class AutomaticLOD : MonoBehaviour
{
  ...
  [SerializeField]
  public LevelsToGenerate m_levelsToGenerate = LevelsToGenerate._4;
  ...
  {
    return Mathf.Pow(1.0f - fSlider, 6.0f);
  }
      ...
      if (rootAutomaticLOD.m_LODGroup == null)
      {
        ...
        if (rootAutomaticLOD.m_LODGroup == null)
        {
          rootAutomaticLOD.m_LODGroup = rootAutomaticLOD.gameObject.AddComponent<LODGroup>();
           int count=rootAutomaticLOD.GetNumberOfLevelsToGenerate();
         //  if(rootAutomaticLOD.m_LODGroup.size<count){
                 lods = rootAutomaticLOD.m_LODGroup.GetLODs();
                LOD[]  lod2s = new LOD[count];
                for (int j = 0; j < count; j++)
                {
                    ...
                    lod2s[j]= new LOD(fMaxCameraDistance,new Renderer[]{});
                }
                rootAutomaticLOD.m_LODGroup.SetLODs(lod2s);
         //  }
        }
      }
